...
`
  ...
  const findIP = async () => {
    try {
      ...
      if (ip.trim().split('.').filter((ipNumber) => ipNumber && Number(ipNumber)).length !== 4) {
        alert("Please enter a valid IP address | Example: 8.8.8.8")
        return
      }
      const response = await axios.get(`https://ipinfo.io/${ip}/json`)
      setIpData(response.data)
    } catch (error) {
      alert.error("Error fetching IP address data:", error)
    }
  }
